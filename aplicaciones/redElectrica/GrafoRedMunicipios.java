...
public class GrafoRedMunicipios {
    ...
    public double crearAdyKruskal() {      
        ...
        for (Arista ar: ars) {
            res += ar.getPeso();
            Municipio mun = verticesAMunicipios.recuperar(new Integer(ar.getOrigen()));
            
            ListaConPI<Municipio> list = adyKruskal.recuperar(mun);
            if (list == null) list = new LEGListaConPI();
            adyKruskal.insertar(mun, list);
            
            list.insertar(verticesAMunicipios.recuperar(new Integer(ar.getDestino())));
        }
